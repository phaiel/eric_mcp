...
scope: server
status: applied
whenToUse: At the start of any tool use on this MCP server — know what is connected before choosing tools.
---

This is the Personal OS MCP server. User-facing playbooks (capture, briefs, formatting) live in the Claude Project — not here. These rules are the server contract only.
