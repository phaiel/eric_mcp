...
Everything else mutating — including `API-patch-page`, `API-move-page`, `API-post-page` outside Inbox, Hevy writes, and `gcal_create_event` / `gcal_update_event` / `gcal_delete_event` — propose in one plain-language line and wait for approval. Batch related writes into one proposal.
